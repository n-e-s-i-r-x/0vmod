...
async function tryWiki(q) {
  try {
    ...
    const hits = (await s.json())?.query?.search || []; if (!hits.length) return null; const results = [];
    for (const h of hits.slice(0, 3)) { try { const e = await fetch('https://en.wikipedia.org/w/api.php?' + new URLSearchParams({ action: 'query', prop: 'extracts|info', exintro: 'true', explaintext: 'true', titles: h.title, format: 'json', inprop: 'url', origin: '*' }), { headers: { 'User-Agent': 'MCMod/3' } }); const p = Object.values((await e.json())?.query?.pages || {})[0]; if (p?.extract?.length > 50) results.push({ title: p.title, url: p.fullurl || '', snippet: p.extract.replace(/\n{2,}/g, '\n').trim().slice(0, 500) }); } catch (_) {} }
