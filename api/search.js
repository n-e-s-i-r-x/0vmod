export async function search(query) {
  ...
  try {
    ...
    const r = await fetch(`https://api.duckduckgo.com/?${params}`, {
      headers: { 'User-Agent': 'MCBedrockBuilder/4.0' }
    });
    const d = await r.json();
    const results = [];
    if (d.AbstractText?.length > 30) {
      results.push({ title: d.Heading || '', url: d.AbstractURL || '', snippet: d.AbstractText.slice(0, 400) });
    }
